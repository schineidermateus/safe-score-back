...
final readonly class ListImportErrors
{
    ...
    public function execute(int $id, int $page, int $perPage): array
    {
        ...
        return ['items' => array_map(static fn ($row): array => ImportRowOutput::fromEntity($row)->toArray(), $this->rows->listErrors($organization, $batch, $page, $perPage)), 'total' => $this->rows->countErrors($organization, $batch), 'page' => $page, 'per_page' => $perPage];
    }
}
